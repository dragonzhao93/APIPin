	...
	const renderPlayControls = () => (
		<div className="flex items-center justify-center gap-3 md:gap-6">
			...
			>
				{isLoading ? (
					<div
						className="w-5 h-5 md:w-6 md:h-6 border-3 border-white/30 
			...
			<AnimatePresence mode="wait">
				{expanded ? (
					...
					>
						...
						<div
							className="flex-1 flex flex-col md:flex-row gap-6 overflow-hidden items-center justify-center"
						...
						<div className="h-[120px] flex flex-col justify-end py-4">
							<div className="flex items-center gap-2 px-2">
								<Text className="w-[45px] text-right text-xs flex-shrink-0">
								...
								<ProgressSlider
									...
									disabled={!currentSong}
								/>
								<Text className="w-[45px] text-xs flex-shrink-0">
									{formatTime(duration)}
								</Text>
							</div>
					...
					>
						{/* 主内容区域 */}
						<div
							className="grid grid-cols-[auto_1fr_auto] md:grid-cols-[auto_1fr_auto] 
						...
						>
							...
							{/* 右侧：播放控制 */}
							<div className="flex-shrink-0">{renderPlayControls()}</div>
						</div>
